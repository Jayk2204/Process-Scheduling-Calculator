function generateProcessTable() {
    ...
    // Create rows for each process with input fields for AT, BT, and Priority, output will be filled later
    for (let i = 1; i <= numProcesses; i++) {
        ...
        row.innerHTML = `
            ...
            <td><input type="number" id="priority${i}" value=""></td>
            <td id="ct${i}"></td>
            <td id="tat${i}"></td>
            <td id="wt${i}"></td>
        `;
...
function calculatePriority() {
    ...
    for (let i = 1; i <= numProcesses; i++) {
        ...
        priority.push(parseInt(document.getElementById('priority' + i).value) || 0);
    ...
    for (let i = 0; i < numProcesses; i++) {
        ...
        // Find the process with the highest priority (lowest priority number) among those that have arrived
        for (let j = 0; j < numProcesses; j++) {
            if (at[j] <= currentTime && !completed[j] && priority[j] < highestPriority) {
